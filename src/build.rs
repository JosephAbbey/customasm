...
pub fn generate_git_info() {
    ...
    let hash = run_command_and_get_output(&mut command_hash).unwrap_or("??????".to_string());
    ...
    let date = run_command_and_get_output(&mut command_date).unwrap_or("????-??-??".to_string());

    println!("cargo:rustc-env=CUSTOMASM_COMMIT_DATE={}", date);
}

fn run_command_and_get_output(command: &mut std::process::Command) -> Result<String, ()> {
    let output = command
