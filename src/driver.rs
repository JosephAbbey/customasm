...
pub fn drive_from_commandline(
    ...
    let maybe_command = parse_command(&mut report, args);

    if let Ok(command) = maybe_command {
        let maybe_result = assemble_with_command(&mut report, fileserver, &command);

        report.print_all(&mut std::io::stderr(), fileserver, command.use_colors);
...
pub fn drive(
    ...
    let command = parse_command(report, args)?;

    let maybe_result = assemble_with_command(report, fileserver, &command);
...
fn assemble_with_command(
    ...
    let assembly = asm::assemble(report, &command.opts, fileserver, &command.input_filenames);

    let output = assembly.output.as_ref().ok_or(())?;
    ...
    for output_group in &command.output_groups {
        if let Some(format) = &output_group.format {
            ...
            if output_group.printout {
                ...
                fileserver.write_bytes(report, None, &output_filename, &formatted)?;
            }
        }
    }
...
fn parse_command(report: &mut diagn::Report, args: &Vec<String>) -> Result<Command, ()> {
    let args_groups = args[1..].split(|arg| arg == "--").collect::<Vec<_>>();
    ...
    for arg_group in args_groups {
        ...
        if let Some(format_str) = parsed.opt_str("f").as_ref() {
            group.format = Some(parse_output_format(report, &format_str)?);
        }

        if let Some(output_filename) = parsed.opt_str("o").as_ref() {
            group.output_filename = Some(output_filename.clone());
        }
        ...
        for define_arg in parsed.opt_strs("d") {
            command
                .opts
                .driver_symbol_defs
                .push(parse_define_arg(report, &define_arg)?);
        }

        command.opts.debug_iterations |= parsed.opt_present("debug-iters");

        command.opts.optimize_statically_known &= !parsed.opt_present("debug-no-optimize-static");
    ...
    for group in &mut command.output_groups {
        if group.format.is_none() {
            if group.printout {
                group.format = Some(OutputFormat::Annotated { base: 16, group: 2 });
            } else {
...
pub fn parse_output_format(
    ...
    for param in &split[1..] {
        ...
        if param_split.len() == 1 {
            ...
            report.error(format!("invalid format argument `{},{}`", format_id, param));

            return Err(());
        }
    }

    let get_arg_str = &mut |params: &mut std::collections::HashMap<String, String>,
                            report: &mut diagn::Report,
                            param_id: &str,
                            def: &str| {
    ...
    let get_arg_usize =
        &mut |params: &mut std::collections::HashMap<String, String>,
              report: &mut diagn::Report,
              param_id: &str,
              def: usize,
              validate: &mut dyn FnMut(usize) -> bool| {
            match params.get(param_id) {
                None => Ok(def),
                Some(value) => {
    ...
    let check_nonzero = &mut |value: usize| -> bool { value > 0 };

    let check_valid_base =
        &mut |base: usize| -> bool { [2, 4, 8, 16, 32, 64, 128].contains(&base) };

    let check_2_or_16 = &mut |base: usize| -> bool { [2, 16].contains(&base) };

    let check_8_16_or_32 = &mut |base: usize| -> bool { [8, 16, 32].contains(&base) };
    ...
    let format = {
        match format_id {
            ...
            "annotatedhex" => OutputFormat::Annotated { base: 16, group: 2 },

            "annotatedbin" => OutputFormat::Annotated { base: 2, group: 8 },
            ...
            "tcgamebin" => OutputFormat::TCGame { base: 2, group: 8 },

            "symbols" => OutputFormat::Symbols,
            "mesen-mlb" => OutputFormat::SymbolsMesenMlb,

            _ => {
                report.error(format!("unknown format `{}`", format_id));
...
fn parse_define_arg(report: &mut diagn::Report, raw_str: &str) -> Result<asm::DriverSymbolDef, ()> {
    let split = raw_str.split('=').collect::<Vec<_>>();
    ...
    if split.len() != 2 {
        report.error(format!("invalid define argument `{}`", raw_str));
    ...
    let value = {
        if value_str == "true" {
            ...
            match maybe_value {
                Ok(value) => expr::Value::make_integer(if has_negative_sign {
                    value.neg()
                } else {
                    value
                }),
                ...
                Err(()) => {
                    report.error(format!("invalid value for define `{}`", name));
    ...
    Ok(asm::DriverSymbolDef { name, value })
}
...
pub fn format_output(
    ...
    let text = {
        match format {
            OutputFormat::Binary => return output.format_binary(),

            OutputFormat::Annotated { base, group } => {
                output.format_annotated(fileserver, *base, *group)
            }

            OutputFormat::TCGame { base, group } => output.format_tcgame(fileserver, *base, *group),
...
fn print_usage(use_colors: bool) {
    ...
    for line in usage_str.lines() {
        if line.starts_with("# ") {
            ...
            let cleaned = line.get("* `".len()..).unwrap().replace("`", "");

            styler.white();
            styler.add("  ");
            styler.add_styled(&cleaned, " -- ", &mut |s| s.cyan(), &mut |s| s.white());
            styler.addln("");
            styler.reset();
        } else {
            styler.gray();
