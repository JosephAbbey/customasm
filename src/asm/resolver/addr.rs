...
pub fn resolve_addr(
    ...
    let value = value.expect_error_or_bigint(report, ast_addr.expr.span())?;
    ...
    if addr.address != prev_value {
        ...
        if ctx.is_last_iteration {
            report.error_span("address did not converge", ast_addr.expr.span());
        }
    ...
    if ctx.is_last_iteration {
        ...
        if addr.address < bank.addr_start {
            report.error_span("address is out of bank range", ast_addr.expr.span());
        ...
        let addr_delta = addr_size
            ...
            .checked_into::<usize>(report, ast_addr.header_span)?;
