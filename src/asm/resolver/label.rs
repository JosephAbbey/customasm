...
pub fn resolve_label(
    ...
    if let asm::AstSymbolKind::Label = ast_symbol.kind {
        let value = ctx.eval_address(report, ast_symbol.decl_span, defs, ctx.can_guess())?;
        ...
        if symbol.value != prev_value {
            ...
            if ctx.is_last_iteration {
                report.error_span("label address did not converge", ast_symbol.decl_span);
            }

            if opts.debug_iterations {
                println!("label: {} = {:?}", ast_symbol.name, symbol.value);
            }
