...
pub fn resolve_align(
    ...
    let value = value.expect_error_or_usize(report, ast_align.expr.span())?;

    let value = {
        match value {
            expr::Value::Integer(bigint) => {
                bigint.checked_into::<usize>(report, ast_align.header_span)?
            }
    ...
    if align.align_size != prev_value {
        ...
        if ctx.is_last_iteration {
            report.error_span("alignment size did not converge", ast_align.expr.span());
        }
    ...
    if ctx.is_last_iteration {
        if align.align_size == 0 {
            report.error_span("invalid alignment size", ast_align.expr.span());
