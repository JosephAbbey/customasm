...
pub fn resolve_constants_simple(
    ...
    let mut iter = asm::ResolveIterator::new(ast, defs, true, false);

    while let Some(ctx) = iter.next_simple(report, decls, defs)? {
        let asm::ResolverNode::Symbol(ast_symbol) = ctx.node else {
            continue;
        };

        let asm::AstSymbolKind::Constant(_) = ast_symbol.kind else {
            continue;
        };

        let resolution_state = resolve_constant_simple(report, opts, ast_symbol, decls, defs)?;
...
fn resolve_constant_simple(
    ...
    let value = asm::resolver::eval_simple(report, decls, defs, &ast_const.expr)?;
    ...
    // in the first pass
    if opts.optimize_statically_known && symbol.value_statically_known {
        if opts.debug_iterations {
            println!("const: {} = {:?} [static]", ast_symbol.name, symbol.value);
        }
    ...
    if opts.debug_iterations {
        println!("const: {} = {:?}", ast_symbol.name, symbol.value);
    }
...
pub fn resolve_constant(
    ...
    // statically known that the encoding can be resolved
    // in the first pass
    if opts.optimize_statically_known && ctx.is_first_iteration && symbol.value_statically_known {
        if opts.debug_iterations {
            println!("const: {} = {:?} [static]", ast_symbol.name, symbol.value);
        }
    ...
    if symbol.value != prev_value {
        ...
        if ctx.is_last_iteration {
            report.error_span("constant value did not converge", ast_symbol.decl_span);
        }
