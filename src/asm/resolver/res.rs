...
pub fn resolve_res(
    ...
    let value = value.expect_error_or_bigint(report, ast_res.expr.span())?;

    let value = {
        match value {
            expr::Value::Integer(bigint) => {
                bigint.checked_into::<u32>(report, ast_res.expr.span())?
            }
    ...
    res.reserve_size = <u32 as TryInto<usize>>::try_into(value).unwrap() * bank.addr_unit;

    if res.reserve_size != prev_value {
        // On the final iteration, unstable guesses become errors
        if ctx.is_last_iteration {
            report.error_span("reserve size did not converge", ast_res.expr.span());
        }
