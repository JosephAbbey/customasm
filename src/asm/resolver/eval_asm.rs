    ...
    for node in &query.ast.nodes {
        if let asm::AstAny::Symbol(ast_symbol) = node {
            if !matches!(ast_symbol.kind, asm::AstSymbolKind::Label) {
                query
                    .report
                    .error_span("only labels are permitted in `asm` blocks", node.span());
            ...
            labels.insert(ast_symbol.name.clone(), expr::Value::Unknown);
...
fn resolve_iteratively(
    ...
    while iter_count < max_iterations {
        ...
        let is_last_iteration = iter_count == max_iterations && ctx.is_last_iteration;
    ...
    query
        .report
        .error_span("`asm` block did not converge", query.span);
...
fn resolve_once(
    ...
    for node in &query.ast.nodes {
        ...
        if let asm::AstAny::Symbol(ast_symbol) = node {
            let cur_address = inner_ctx.eval_address(query.report, query.span, defs, true)?;
            ...
            labels.insert(ast_symbol.name.clone(), new_value);
        } else if let asm::AstAny::Instruction(ast_instr) = node {
            let substs = parse_substitutions(query.report, ast_instr.span, &ast_instr.src)?;

            let new_excerpt = perform_substitutions(&ast_instr.src, &substs, query)?;

            // Run the matcher algorithm
            let mut matches = asm::matcher::match_instr(opts, defs, ast_instr.span, &new_excerpt);

            let attempted_match_excerpt = {
                if substs.len() == 0 {
                    None
                } else {
                    Some(format!("match attempted: `{}`", new_excerpt))
                }
            };
...
fn parse_substitutions<'excerpt>(
    ...
    let mut walker = syntax::Walker::new(excerpt, span.file_handle, span.location().unwrap().0);

    while !walker.is_over() {
        walker.skip_ignorable();

        if let Some(tk_brace_open) = walker.maybe_expect(syntax::TokenKind::BraceOpen) {
            let start = walker.get_index_at_span_start(tk_brace_open.span);
