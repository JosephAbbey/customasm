...
type WorkingMatch<'src> = (InstructionMatch, syntax::Walker<'src>);
...
impl InstructionArgument {
    pub fn is_same(&self, other: &InstructionArgument) -> bool {
        ...
        match (&self.kind, &other.kind) {
            (InstructionArgumentKind::Expr(_), InstructionArgumentKind::Expr(_)) => true,

            (InstructionArgumentKind::Nested(ref a), InstructionArgumentKind::Nested(ref b)) => {
                a.is_same(b)
            }
...
pub fn match_all(
    ...
    for any_node in &ast.nodes {
        if let asm::AstAny::Instruction(ast_instr) = any_node {
            let mut matches = match_instr(opts, defs, ast_instr.span, &ast_instr.src);

            if let Err(()) = error_on_no_matches(report, ast_instr.span, &matches) {
                continue;
            }
            ...
            for mtch in &mut matches {
                mtch.encoding_statically_known =
                    get_match_statically_known(decls, defs, symbol_ctx, &mtch);

                mtch.encoding_size = get_match_static_size(defs, &mtch).unwrap_or(0);
            }
            ...
            instr.encoding_statically_known =
                instr.matches.iter().all(|m| m.encoding_statically_known);

            // Statically calculate the encoding size
            // with a pessimistic guess
            let largest_encoding = instr
                .matches
                .iter()
                .max_by_key(|m| m.encoding_size)
                .unwrap();

            instr.encoding = util::BigInt::new(0, Some(largest_encoding.encoding_size));
            ...
            if opts.debug_iterations {
                println!(
                    ...
                    if instr.encoding_statically_known {
                        " [static]"
                    } else {
                        ""
                    }
...
pub fn error_on_no_matches(
    ...
    if matches.len() == 0 {
        report.error_span("no match found for instruction", span);
...
fn get_match_statically_known(
    ...
    let query_variable = |query: &expr::StaticallyKnownVariableQuery| match decls
        .symbols
        .try_get_by_name(symbol_ctx, query.hierarchy_level, query.hierarchy)
    {
        None => false,
        Some(symbol_ref) => {
        ...
        match param.typ {
            ...
            asm::RuleParameterType::RuledefRef(_) => {
                if let asm::InstructionArgumentKind::Nested(ref nested_match) = arg.kind {
                    if get_match_statically_known(decls, defs, symbol_ctx, nested_match) {
                        provider.locals.insert(
                            param.name.clone(),
                            expr::StaticallyKnownLocal {
                                value_known: true,
...
fn get_match_static_size(defs: &asm::ItemDefs, mtch: &asm::InstructionMatch) -> Option<usize> {
    let ruledef = defs.ruledefs.get(mtch.ruledef_ref);
    let rule = &ruledef.get_rule(mtch.rule_ref);
    ...
    for i in 0..rule.parameters.len() {
        ...
        match param.typ {
            ...
            asm::RuleParameterType::RuledefRef(_) => {
                if let asm::InstructionArgumentKind::Nested(ref nested_match) = arg.kind {
                    let maybe_nested_size = get_match_static_size(defs, nested_match);
...
pub fn match_instr(
    ...
    let mut walker = syntax::Walker::new(src, span.file_handle, span.location().unwrap().0);

    if opts.optimize_instruction_matching {
        let ruledef_matches = match_with_ruledef_map(defs, walker);
        ...
        for i in 0..defs.ruledefs.defs.len() {
            ...
            let ruledef_matches = match_with_ruledef(defs, ruledef_ref, &mut walker, true);
    ...
    let mut matches = working_matches.into_iter().map(|m| m.0).collect::<Vec<_>>();
    ...
    // each match
    for mtch in &mut matches {
        mtch.exact_part_count = get_recursive_exact_part_count(defs, mtch);
    }
    ...
    for part_index in at_pattern_part..rule.pattern.len() {
        ...
        match part {
            ...
            asm::RulePatternPart::Whitespace => {
                if !walker.is_over() && walker.next_token().kind != syntax::TokenKind::Whitespace {
                    return vec![];
                }
            }
            ...
            asm::RulePatternPart::ParameterIndex(param_index) => {
                ...
                match param.typ {
                    ...
                    asm::RuleParameterType::RuledefRef(ruledef_ref) => {
                        ...
                        // Try both with and without lookahead character
                        for enable_lookahead in [false, true] {
                            result.extend(match_with_nested_ruledef(
                                defs,
                                ruledef_ref,
                                rule,
                                walker.clone(),
...
fn match_with_expr<'src>(
    ...
    let Some(maybe_expr) = parse_with_lookahead(
        &rule.pattern,
        at_pattern_part,
        enable_lookahead,
        &mut walker,
    ...
    let span = walker.get_span(walker_start, walker_end);

    let excerpt = walker.get_excerpt(walker_start, walker_end);
...
fn match_with_nested_ruledef<'src>(
    ...
    let Some(nested_matches) = parse_with_lookahead(
        &rule.pattern,
        at_pattern_part,
        enable_lookahead,
        &mut walker,
        |walker| match_with_ruledef(defs, nested_ruledef_ref, walker, false),
    ) else {
        return vec![];
    };
...
where
    F: FnOnce(&mut syntax::Walker<'src>) -> T,
{
    if !enable_lookahead {
        return Some(parse_fn(walker));
    }

    let maybe_lookahead_char = find_lookahead_char(pattern, at_pattern_part);

    if let Some(lookahead_char) = maybe_lookahead_char {
        let maybe_limit = walker.find_lookahead_char_index(lookahead_char);
...
/// it returns `None`.
fn find_lookahead_char(pattern: &[asm::RulePatternPart], at_pattern_part: usize) -> Option<char> {
...
fn get_recursive_exact_part_count(defs: &asm::ItemDefs, instr_match: &InstructionMatch) -> usize {
    let mut count = 0;

    for arg in &instr_match.args {
        if let InstructionArgumentKind::Nested(ref nested_match) = arg.kind {
            count += get_recursive_exact_part_count(defs, nested_match);
        }
    }
