...
mod directive_fn;
pub use directive_fn::{AstDirectiveFn, AstFnParameter};
...
pub use directive_ruledef::{
    AstDirectiveRuledef, AstRule, AstRuleParameter, AstRuleParameterType, AstRulePatternPart,
};

mod fields;
pub use fields::{AstField, AstFields};

mod instruction;
pub use instruction::AstInstruction;

mod symbol;
pub use symbol::{AstSymbol, AstSymbolConstant, AstSymbolKind};
...
{
    let mut result = AstTopLevel { nodes: Vec::new() };
    ...
    S: std::borrow::Borrow<str>,
{
    if once_filenames.contains(root_filename.borrow()) {
        return Ok(AstTopLevel { nodes: Vec::new() });
    }

    let file_handle = fileserver.get_handle(report, span, root_filename.borrow())?;

    let src = fileserver.get_str(report, span, file_handle)?;

    let mut walker = syntax::Walker::new(&src, file_handle, 0);
    ...
    if root_ast
        .nodes
        .iter()
        .any(|n| matches!(n, AstAny::DirectiveOnce(_)))
    {
    ...
    while node_index < root_ast.nodes.len() {
        ...
        if let AstAny::DirectiveInclude(ast_include) = node {
            ...
            if seen_filenames.contains(&included_filename) {
                report.error_span("recursive file inclusion", ast_include.filename_span);
            ...
            root_ast
                .nodes
                .splice(node_index..(node_index + 1), inner_ast.nodes);
...
pub fn parse(report: &mut diagn::Report, walker: &mut syntax::Walker) -> Result<AstTopLevel, ()> {
...
pub fn parse_nested_toplevel(
    ...
    while !walker.is_over() && !walker.next_useful_is(0, syntax::TokenKind::BraceClose) {
        if let Some(node) = parse_line(report, walker)? {
            nodes.push(node);
        }
    }
...
fn parse_line(
    ...
    // Everything else is regarded as an instruction
        ...
        Ok(Some(AstAny::Instruction(instruction::parse(
            report, walker,
        )?)))
    }
}
