...
impl<'src> Walker<'src> {
    ...
    pub fn slice(&self, start_byte_index: usize, end_byte_index: usize) -> Walker<'src> {
    ...
    pub fn advance_to_token_end(&mut self, token: &syntax::Token) {
    ...
    pub fn get_index_at_span_start(&self, span: diagn::Span) -> usize {
        span.location().unwrap().0 - self.span_offset
    }

    pub fn get_index_at_span_end(&self, span: diagn::Span) -> usize {
    ...
    pub fn get_span(&self, start_byte_index: usize, end_byte_index: usize) -> diagn::Span {
        let start = self.span_offset + start_byte_index;

        let end = self.span_offset + end_byte_index;

        diagn::Span::new(self.file_handle, start, end)
    }
    ...
    pub fn get_excerpt(&self, start_byte_index: usize, end_byte_index: usize) -> &'src str {
    ...
    pub fn get_span_excerpt(&self, span: diagn::Span) -> &'src str {
        let start = self.get_index_at_span_start(span);
        let end = self.get_index_at_span_end(span);
        &self.src[start..end]
    }

    pub fn get_cursor_limit(&self) -> usize {
        self.cursor_limit
    }

    pub fn set_cursor_limit(&mut self, end: usize) {
        self.cursor_limit = end;
    }

    fn char_at(&self, byte_index: usize) -> char {
    ...
    fn token_at(&self, byte_index: usize) -> syntax::Token {
        if byte_index >= self.cursor_limit {
            let span_index = self.span_offset + self.cursor_limit;

            let span = diagn::Span::new(self.file_handle, span_index, span_index);
        ...
        syntax::Token { kind, span }
    }

    pub fn next_token(&self) -> syntax::Token {
        self.token_at(self.cursor_index)
    }

    pub fn next_nth_token(&self, mut nth: usize) -> syntax::Token {
    ...
    pub fn next_nth_useful_token(&self, mut nth: usize) -> syntax::Token {
        let mut byte_index = self.cursor_index;
    ...
    fn next_useful_token(&self) -> syntax::Token {
        self.next_nth_useful_token(0)
    }

    pub fn next_linebreak(&self) -> Option<syntax::Token> {
        let mut byte_index = self.cursor_index;
    ...
    pub fn next_useful_is(&mut self, nth: usize, kind: syntax::TokenKind) -> bool {
        let token = self.next_nth_useful_token(nth);
        token.kind == kind
    }

    pub fn maybe_expect(&mut self, kind: syntax::TokenKind) -> Option<syntax::Token> {
        let token = self.next_useful_token();
        if token.kind == kind {
    ...
    pub fn expect_linebreak(&mut self, report: &mut diagn::Report) -> Result<(), ()> {
        match self.maybe_expect_linebreak() {
            ...
            None => {
                report.error_span(
                    format!("expected {}", syntax::TokenKind::LineBreak.printable()),
                    self.get_cursor_span(),
                );
    ...
    pub fn advance_until_closing_brace(&mut self) -> Walker<'src> {
    ...
    pub fn advance_until_linebreak(&mut self) -> Walker<'src> {
        let start = self.cursor_index;
        ...
        while !self.is_over() {
            ...
            if token.kind == syntax::TokenKind::LineBreak && brace_nesting == 0 {
                break;
    ...
    pub fn find_lookahead_char_index(&self, wanted_char: char) -> Option<usize> {
        let mut byte_index = self.cursor_index;
