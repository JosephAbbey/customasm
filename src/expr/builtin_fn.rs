...
pub fn eval_builtin_sizeof(query: &mut expr::EvalFunctionQuery) -> Result<expr::Value, ()> {
    query.ensure_arg_number(1)?;

    let (_bigint, size) = query.args[0]
        .value
        .expect_sized_integerlike(query.report, query.args[0].span)?;
...
pub fn eval_builtin_le(query: &mut expr::EvalFunctionQuery) -> Result<expr::Value, ()> {
    ...
    if bigint.size.unwrap() % 8 != 0 {
        ...
        query
            .report
            .note(format!("got size {}", bigint.size.unwrap()));
