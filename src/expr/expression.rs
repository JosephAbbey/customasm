...
impl Value {
    ...
    pub fn coallesce_to_integer<'a>(&'a self) -> std::borrow::Cow<'a, expr::Value> {
        match self {
            Value::String(ref s) => std::borrow::Cow::Owned(expr::Value::Integer(s.to_bigint())),

            _ => std::borrow::Cow::Borrowed(self),
        }
    }

    pub fn unwrap_bigint(&self) -> &util::BigInt {
        match self {
    ...
    pub fn expect_bigint(
        ...
        match self {
            ...
            Value::Unknown => {
                report.error_span("value is unknown", span);

                Err(())
            }

            _ => {
                report.error_span("expected integer", span);
    ...
    pub fn expect_sized_bigint(
        ...
        match bigint.size {
            ...
            None => {
                report.error_span("expected integer with definite size", span);
    ...
    pub fn expect_sized_integerlike(
        ...
        if let Some(bigint) = self.coallesce_to_integer().get_bigint() {
            ...
            report.error_span("value has no definite size", span);

            return Err(());
        }

        report.error_span("expected integer-like value with definite size", span);
    ...
    pub fn expect_error_or_bigint(
        ...
        match self.coallesce_to_integer().as_ref() {
            value @ expr::Value::Unknown | value @ expr::Value::FailedConstraint(_) => {
                Ok(value.to_owned())
            }

            value @ expr::Value::Integer(_) => Ok(value.to_owned()),
    ...
    pub fn expect_error_or_bool(
        ...
        match self.coallesce_to_integer().as_ref() {
            ...
            value @ expr::Value::Bool(_) => Ok(value.to_owned()),

            _ => {
                report.error_span("expected boolean", span);
    ...
    pub fn as_usize(&self) -> Option<usize> {
        match self {
            Value::Integer(ref bigint) => bigint.maybe_into::<usize>(),

            _ => None,
        }
    }

    pub fn expect_usize(&self, report: &mut diagn::Report, span: diagn::Span) -> Result<usize, ()> {
        match self {
            Value::Integer(ref bigint) => bigint.checked_into::<usize>(report, span),
            ...
            _ => {
                report.error_span("expected non-negative integer", span);
    ...
    pub fn expect_error_or_usize(
        ...
        match &self {
            expr::Value::Unknown | expr::Value::FailedConstraint(_) => Ok(self),

            Value::Integer(ref bigint) => {
                bigint.checked_into::<usize>(report, span)?;
    ...
    pub fn expect_nonzero_usize(
        ...
        match self {
            Value::Integer(ref bigint) => bigint.checked_into_nonzero_usize(report, span),
            ...
            _ => {
                report.error_span("expected positive integer", span);
    ...
    pub fn expect_bool(&self, report: &mut diagn::Report, span: diagn::Span) -> Result<bool, ()> {
        match self {
    ...
    pub fn expect_string(
        ...
        match self {
            ...
            _ => {
                report.error_span("expected string", span);
...
impl ExprString {
    pub fn to_bigint(&self) -> util::BigInt {
        match &*self.encoding {
            ...
            "ascii" => {
                let units = self.utf8_contents.chars();
                let bytes = units.map(|c| if c as u32 >= 0x100 { 0x00 } else { c as u8 });
