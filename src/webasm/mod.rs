...
#[no_mangle]
pub unsafe extern "C" fn wasm_assemble(format_str: *const String, src: *mut String) -> *mut String {
    ...
    let output = {
        match assembly.output {
            ...
            None => {
                ...
                return wasm_string_new_with(String::from_utf8(err).unwrap());
            }
        }
    };

    let format = driver::parse_output_format(&mut report, format_str).unwrap();
...
pub unsafe extern "C" fn wasm_string_set_byte(s: *mut String, index: u32, value: u8) {
    ...
    std::ptr::write(
        std::mem::transmute::<_, *mut u8>(bytes).offset(index as isize),
        value,
    )
}
